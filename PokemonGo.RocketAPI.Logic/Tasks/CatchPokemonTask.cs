...
namespace PokemonGo.RocketAPI.Logic.Tasks
{
    public class CatchPokemonTask
    {
        ...
        public static async Task<ItemId> GetBestBall(dynamic encounter, float probability)
        {
            ...
            if (ultraBalls && (pokemonCp >= 1000 || (iV >= Logic._client.Settings.TransferPokemonKeepIV && probability < 0.40)))
                return ItemId.ItemUltraBall;

            if (greatBalls && (pokemonCp >= 300 || (iV >= Logic._client.Settings.TransferPokemonKeepIV && probability < 0.50)))
        ...
        public static async Task<ItemId> GetBestBerry(dynamic encounter, float probability)
        {
            ...
            if (nanabBerryCount > 0 && (pokemonCp >= 1000 || (iV >= Logic._client.Settings.TransferPokemonKeepIV && probability < 0.40)))
                return ItemId.ItemNanabBerry;

            if (blukBerryCount > 0 && (pokemonCp >= 500 || (iV >= Logic._client.Settings.TransferPokemonKeepIV && probability < 0.50)))
